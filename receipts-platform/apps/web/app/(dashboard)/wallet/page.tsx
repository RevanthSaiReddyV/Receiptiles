...
export default function DashboardWalletPage() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-zinc-900">Wallet Pass</h1>
      ...
      <div className="rounded-2xl bg-white border border-zinc-200 shadow-sm p-6">
        <h2 className="text-sm font-semibold text-zinc-900 mb-4">Add to this device</h2>
        <div className="flex flex-wrap gap-3">
          {(platform === "ios" || platform === "desktop") && (
            <button
              onClick={() => handleAdd("apple")}
              disabled={isAdding}
              className="px-5 py-3 bg-black text-white rounded-xl text-sm font-medium border border-zinc-800 hover:bg-zinc-800 transition-all disabled:opacity-50 flex items-center gap-2"
            >
          ...
          {(platform === "android" || platform === "desktop") && (
            <button
              ...
              disabled={isAdding}
              className="px-5 py-3 bg-zinc-100 text-black rounded-xl text-sm font-medium border border-zinc-300 hover:bg-zinc-200 transition-all disabled:opacity-50 flex items-center gap-2"
            >
      ...
      {devices.length > 0 && (
        <div className="rounded-2xl bg-white border border-zinc-200 shadow-sm p-6">
          <h2 className="text-sm font-semibold text-zinc-900 mb-4">Connected Devices ({devices.length})</h2>
          <div className="space-y-3">
            {devices.map((d) => (
              <div key={d.id} className="flex items-center justify-between p-3 rounded-xl bg-zinc-50 border border-zinc-200">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-sm text-zinc-900 font-medium">{d.deviceName || "Unknown Device"}</div>
      ...
      <div className="rounded-2xl bg-emerald-50 border border-emerald-200 p-6">
        <h3 className="text-sm font-semibold text-emerald-700 mb-2">How it works</h3>
