...
function ReceiptAIAnalyzer() {
  ...
  const analyzeReceipt = async () => {
    ...
    try {
      const res = await fetch('/api/receipt-demo', {
        method: 'POST',
      ...
      <nav className={`fixed top-0 w-full z-50 transition-all duration-500 ${scrolled ? 'bg-[#F7F6F2]/90 backdrop-blur-md border-b border-[#EBEAE4]' : 'bg-transparent border-b-transparent'}`}>
        <div className={`max-w-7xl mx-auto px-6 flex items-center justify-between transition-all duration-500 ${scrolled ? 'h-20' : 'h-28'}`}>
          ...
          <div className="flex items-center space-x-4">
            <a href="/login" className={`hidden sm:block text-sm font-semibold transition-colors ${scrolled ? 'text-[#1C1C1A] hover:text-[#4A5D4E]' : 'text-white hover:text-white/80'}`}>Sign In</a>
            <a href="/signup"><ShimmerButton variant={scrolled ? "dark" : "outline"} size="sm">Sign Up</ShimmerButton></a>
          </div>
        </div>
      </nav>

      {/* Hero */}
      <section className="relative pt-32 pb-32 lg:pt-40 lg:pb-40 overflow-hidden min-h-screen rounded-b-[2rem] flex flex-col justify-center">
        <video autoPlay loop muted playsInline className="absolute inset-0 h-full w-full object-cover pointer-events-none z-0" src="https://assets.mixkit.co/videos/preview/mixkit-swirling-yellow-and-brown-ink-in-water-10068-large.mp4" />
